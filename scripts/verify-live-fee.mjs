 ...
 * platformFeeWld must equal PLATFORM_FEE_BPS (2%) of the sum of the *quoted*
...
if (getAddress(last.to) !== getAddress(WLD)) {
  ...
  if (amount !== expectedFee) {
    console.log('FAIL: on-chain fee amount != expected 2% of quotes');
    ok = false;
  }
}
